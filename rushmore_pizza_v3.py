...
# -------------------------------
def generate_receipt(order_id, pizza_type, quantity, order_type, total_price, discount_applied, delivery_type, extras, postcode=None, email=None):
    print("------------------------ORDER RECEIPT------------------------")
    print(f"Order Number: {order_id}")  # Random order number
    print(f"Order Type: {order_type}")  # Box or Slice
    print(f"Pizza Type: {pizza_type}")  # Type of pizza ordered
    print(f"Quantity: {quantity}")  # Quantity of items
    print(f"Discount Applied: {'Yes' if discount_applied else 'No'}")  # Discount status
    print(f"Extras: {', '.join(extra['name'] for extra in extras) if extras else 'None'}")  # List extras
    print(f"Delivery Type: {delivery_type}")  # Takeout or Delivery
    ...
    if extras:
        ...
        print(f"{'Subtotal for Extras:'} ${extras_total:.2f}")
    else:
        print("Extras: None")
    print(f"Total Price: ${total_price:.2f}")  # Total price formatted

    if postcode:
        print(f"Postcode: {postcode}")  # Optional postcode

    if email:
        print(f"A receipt has been sent to your {email}.")  # Optional email
...
def handle_box_order(pizza_name, price):
    ...
    # Show receipt
    generate_receipt(order_id, pizza_name, quantity, "Box", total, discount_applied, delivery_type, extras_selected, postcode, email)
...
def handle_slice_order(pizza_name, slice_price):
    ...
    # Show receipt
    generate_receipt(order_id, pizza_name, quantity, "Slice", total, discount_applied, delivery_type, extras_selected, postcode, email)
